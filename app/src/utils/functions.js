...
const exercises = exercisesFlattener(EXERCISES) // flatten the exercises object

export function generateWorkout(args) {
    const { muscles, poison: workout, goal } = args // destructure the args object
    let exer = Object.keys(exercises); // get the keys of the exercises object
    exer = exer.filter((key) => exercises[key].meta.environment !== "home"); // filter the exercises object to only include exercises that are not for home
    let includedTracker = []; 
    let numSets = 5;
    let listOfMuscles;
    ...
    listOfMuscles = new Set(shuffleArray(listOfMuscles)); // shuffle the array of muscles and convert it to a set
    let arrOfMuscles = Array.from(listOfMuscles); // convert the set to an array
    let scheme = goal 
    let sets = SCHEMES[scheme].ratio
        .reduce((acc, curr, index) => {
            //make this compound and exercise muscle -> array of objects and destructure in loop
            return [
                ...acc, // spread the acc array
                ...[...Array(parseInt(curr)).keys()].map((val) =>
                    index === 0 ? "compound" : "accessory"
                ), // map over the array of integers and return an array of objects
            ];
        }, [])
        .reduce((acc, curr, index) => {
